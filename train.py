...
def main(_):
    ...
    if args.resume:
        output_folder_root = os.path.join(args.experiment_dir, "epoch_300_042719")
    clear_before = not args.resume
    output_folder_root = my_util.check_dir(output_folder_root, clear_before=clear_before)
    ...
    with tf.Session(config=config) as sess:
        model = UNet(args.experiment_dir, output_folder_root, resume=args.resume, batch_size=args.batch_size,
